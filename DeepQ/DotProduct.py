import numpy as np


np.random.seed(0)

X = [[1, 2, 3, 2.5],
     [2.0, 5.0, -1.0, 2.0],
     [-1.5, 2.7, 3.3, -0.8]]


class Layer_Dense:
    def __init__(self, n_inputs, n_neurons):
        self.weights = 0.10 * np.random.randn(n_inputs, n_neurons)
        self.biases = np.zeros((1, n_neurons))

    def forward(self, inputs):
        self.output = np.dot(inputs, self.weights) + self.biases


layer1 = Layer_Dense(4,5)
layer2 = Layer_Dense(5,2) # input of the coming layer must be equal to output of the previous layer

layer1.forward(X)
print(layer1.output)
layer2.forward(layer1.output)
print(layer2.output)


# inputs = [[1, 2, 3, 2.5],
#           [2.0, 5.0, -1.0, 2.0],
#           [-1.5, 2.7, 3.3, -0.8]]
#
# weights = [[0.2, 0.8, -0.5, 1.0],
#            [0.5, -0.91, 0.26, -0.5],
#            [-0.26, -0.27, 0.17, 0.87]]
#
# biases = [2, 3, 0.5]
#
#
# #weights = np.transpose(weights)
# output = np.dot(inputs, np.array(weights).T) + biases
#
# print(output)




# inputs = [1, 2, 3, 2.5]
# weights = [[0.2, 0.8, -0.5, 1.0],
#            [0.5, -0.91, 0.26, -0.5],
#            [-0.26, -0.27, 0.17, 0.87]]
# biases = [2, 3, 0.5]

# layerOutputs = np.empty((0,len(weights)))
# for neuronWeights, neuronBias in zip(weights, biases):
#     neuronOutput = 0
#     for nInput, weight in zip(inputs, neuronWeights):
#         neuronOutput += nInput * weight
#     neuronOutput += neuronBias
#     layerOutputs = np.append(layerOutputs, neuronOutput)

# my way
# for i in range(len(weights)):
#     neuronOutput = 0
#     for j in range(len(inputs)):
#         neuronOutput += inputs[j]*weights[i][j]
#     neuronOutput += biases[i]
#     layerOutputs = np.append(layerOutputs, neuronOutput)
#
# print(layerOutputs)
